...
void main() {
  ...
  test('Should return error if value is less than min size', () {
    final error = systemUnderTest.validate({ 'any_field': faker.randomGenerator.string(4, min: 1) });
    expect(error, ValidationError.invalidField);
  });

  test('Should return null if value is equal the min size', () {
    final error = systemUnderTest.validate({ 'any_field': faker.randomGenerator.string(5, min: 5) });
    expect(error, null);
  });

  test('Should return null if value is bigger than min size', () {
    final error = systemUnderTest.validate({ 'any_field': faker.randomGenerator.string(10, min: 6) });
    expect(error, null);
  });
}
